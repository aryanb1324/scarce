...
it." Every guarantee `data/cifar10.py` claims gets a test here that fails when the
guarantee is violated -- split disjointness, exact class balance, seed
reproducibility, raise-don't-truncate on an oversized budget, subset-derived
normalization statistics, and that the leakage assertion itself actually fires.

These mirror `tests/test_sampling.py` and, like it, run on synthetic labels rather
than downloading CIFAR-10, so the suite stays in milliseconds. The one thing that
cannot be checked synthetically -- that torchvision's arrays land as (N, 3, 32, 32)
uint8 -- is checked by `test_to_chw_layout` on a fake array of the real shape.

Run with: python -m pytest tests/test_cifar10.py -v
"""
...
N_FAKE = 5000        # 500 per class, deliberately shuffled
PER_CLASS = 500
VAL_PC = 100         # scaled-down stand-in for the real VAL_PER_CLASS
...
def test_val_split_is_a_partition(labels):
    ...
    assert val.numel() == VAL_PC * 10
    assert pool.numel() == N_FAKE - VAL_PC * 10
...
def test_val_split_is_class_balanced(labels):
    """The deviation from mnist_v2's uniform permutation: the holdout is stratified,
    so the pool ceiling is exact and seed-independent rather than approximate."""
    _, val = train_val_split(labels, val_per_class=VAL_PC)
    counts = torch.bincount(labels[val], minlength=10)
    assert torch.all(counts == VAL_PC), counts


def test_val_split_seed_constant_is_the_frozen_one():
    """Guards the constant itself. If this changes, every recorded CIFAR result
    stops being comparable, so the change should have to break a test."""
    assert CIFAR_VAL_SPLIT_SEED == 20260728
    assert VAL_PER_CLASS == 500
    assert MAX_SHOTS_PER_CLASS == 4500


def test_val_split_raises_when_a_class_is_too_small(labels):
    with pytest.raises(ValueError):
        train_val_split(labels, val_per_class=PER_CLASS + 1)
...
def test_oversized_request_raises_rather_than_silently_shrinking(labels):
    ...
    # And exactly at the boundary: the pool holds PER_CLASS - VAL_PC per class.
    with pytest.raises(ValueError):
        stratified_subset(labels, pool, n_per_class=PER_CLASS - VAL_PC + 1, seed=0)
    ok = stratified_subset(labels, pool, n_per_class=PER_CLASS - VAL_PC, seed=0)
    assert ok.numel() == (PER_CLASS - VAL_PC) * 10
...
def test_preflight_runs_the_checks_and_returns_every_subset(labels, capsys):
...
def test_preflight_propagates_an_impossible_budget(labels):
    pool, val = train_val_split(labels, val_per_class=VAL_PC)
    with pytest.raises(ValueError):
        preflight(labels, pool, val, shots_per_class=[10, 99_999], seeds=[0],
                  verbose=False)
